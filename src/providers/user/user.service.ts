...
FirebaseObjectObservable, FirebaseAuthState, FirebaseApp } from 'angularfire2';
...
export class UserService extends BaseService{
  ...
  private listenAuthState():void{
    this.af.auth.subscribe((authState:FirebaseAuthState)=>{
      if(authState){
        ...
        .af.database.object(`/users/${authState.auth.uid}`);
  ...
  edit(user:{
    ...
    .update(user).catch(this.handlePromisseError);
  }
